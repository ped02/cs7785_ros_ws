...
class RotateRobotActionServer(Node):
    def __init__(self, **kwargs):
        super(RotateRobotActionServer, self).__init__(
            'rotate_robot_action', **kwargs
        )
        ...
        angle_deadzone_radians_parameter_descriptor = ParameterDescriptor(
            description='Angle deadzone in radians'
        )
        self.declare_parameter(
            'angle_deadzone_radians',
            0.09,
            angle_deadzone_radians_parameter_descriptor,
        )

        parameter_validator['angle_deadzone_radians'] = validate_positive_double
    ...
    def update_parameters(self):
        ...
        self.theta_deadzone_radians = (
            self.get_parameter('angle_deadzone_radians')
            .get_parameter_value()
            .double_value
        )
    ...
    def update_state_odometry(self, odometry: Odometry):
        ...
        # angular_z = odometry.twist.twist.angular.z
        odom_theta = Rotation.from_quat(
            [
        ...
        if dt <= self.odometry_max_period_sec:
            ...
            self.current_theta += odom_theta - self.last_odom_theta
            # self.get_logger().info(f'[{dt}] {odom_theta - self.last_odom_theta}')
    ...
    def stop_robot(self):
        self.command_publisher.publish(Twist(angular=Vector3(z=0.0)))

    def execute_action_callback(self, goal_handle: ServerGoalHandle):
        self.running = True

        goal_request: Control.Goal = goal_handle.request
        target_theta_radians = goal_request.target_value
        target_vector = radians_to_vector(1.0, target_theta_radians)

        feedback_message = Control.Feedback()

        # Controller
        BURGER_MAX_ANGULAR_RATE_RADIANS = 2.84  # rad/s

        CONTROLLER_COMMAND_MIN = BURGER_MAX_ANGULAR_RATE_RADIANS * 0.2
        # CONTROLLER_COMMAND_MAX = BURGER_MAX_ANGULAR_RATE_RADIANS * 0.8
        CONTROLLER_COMMAND_MAX = BURGER_MAX_ANGULAR_RATE_RADIANS * 0.4

        kp = 1 / 0.2  # give 1/5 seconds to correct
        ...
        while self.running:
            current_theta_radians = self.current_theta
            current_time = self.get_clock().now()

            current_vector = radians_to_vector(1.0, current_theta_radians)

            error_theta_radians = signed_angle_difference(
                current_vector, target_vector
            )

            if abs(error_theta_radians) < self.theta_deadzone_radians:
                # Goal Reached
                break

            # Command
            twist_command = Twist()
            twist_command.angular.z = controller.execute(
                error_theta_radians, current_time, previous_time
            )
            self.command_publisher.publish(twist_command)

            # Feedback
            feedback_message.error_value = error_theta_radians
        ...
        result.final_value = self.current_theta
        current_vector = radians_to_vector(1.0, result.final_value)
        result.error_value = signed_angle_difference(
            current_vector, target_vector
        )
...
def main(args=None):
    ...
    executor.add_node(rotate_robot_action_server)

    # Spin Node
    executor.spin()

    # Destroy node
    rotate_robot_action_server.destroy_node()
    rclpy.shutdown()
